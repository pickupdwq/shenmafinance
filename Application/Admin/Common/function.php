...
function pic_show_index($id){
    if($id=='0'){
        return "无";
    }
    $picture =M('picture')->find($id);
    $url = __ROOT__.$picture['path'];
    return "<img style='width:100px;height:100px' src='$url'>";
}

function pic_title_index($pic_title){
    $map['pic_title']=$pic_title;
    $data=M('indexpage')->where($map)->find();
    // dump($data);
    return "<span>".$data['pic_title']."</span>";
}

function pic_detail_index($pic_detail){
    $map['pic_detail']=$pic_detail;
    $data=M('indexpage')->where($map)->find();
    // dump($data);
    return "<span>".$data['pic_detail']."</span>";
}

function is_show_index($is_show){
    return "<span>".$is_show."</span>";
}


function get_company_detail($detail){
    return $detail;
}
...
function set_top($id){
  if(!$id){
    ...
    $info = M('recruit_detail')->find($id);
    $set_top = U('Recruit/top',array('id'=>$id));
    $set_untop = U('Recruit/untop',array('id'=>$id));
    if($info['if_top']==1){
        return "<a href='$set_untop'>取消置顶</a>";
    }else{
        return "<a href='$set_top'>置顶</a>";
    }
}
function recruit_status($id){
    $info=M('recruit_detail')->find($id);
    $recruit_now=U('Recruit/get_now',array('id'=>$id));
    $recruit_stop=U('Recruit/get_stop',array('id'=>$id));
    if($info['status']=='1'){
        return "<a href='$recruit_stop'>停招</a>";
    }
    else{
        return "<a href='$recruit_now'>重新招聘</a>";
    }
}

function show_type($id){
    $type=M('recruit')->find($id);
    if($type){
...
function add_faq($id){
    $addurl=U('Faq/add_faq',array('id'=>$id));
    return "<a href=".$addurl.">新增问题</a>";
}


function edit_question($id){
    $editurl=U('Faq/edit_question',array('id'=>$id));
    return "<a href=".$editurl.">编辑问题</a>";
}